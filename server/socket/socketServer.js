...
class SocketServer {
  ...
  setupEventHandlers() {
    this.io.on("connection", (socket) => {
      ...
      if (socket.user.role !== "user" && socket.user.role !== "moderator") {
        socket.join(`agents_${socket.user.role}`);
      }

      const isModerator = socket.user.role === "moderator";

      // Moderators receive only the existing live chat socket handlers.
      if (!isModerator) {
    ...
    socket.on("subscribe_live_scores", (sports) => {
      if (Array.isArray(sports)) {
        sports.forEach((sport) => {
          ...
          console.log(
            `User ${socket.userId} subscribed to ${sport} live scores`,
          );
        });
      }
    });
    ...
    socket.on("unsubscribe_live_scores", (sports) => {
      if (Array.isArray(sports)) {
        sports.forEach((sport) => {
          ...
          console.log(
            `User ${socket.userId} unsubscribed from ${sport} live scores`,
          );
        });
      }
    });
  ...
  handleChatAdvancedEvents(socket) {
    socket.on("typing_start", async (payload = {}) => {
      await this.forwardConversationEvent(
        socket,
    ...
    socket.on("message_delivered", async (payload = {}, callback) => {
      try {
        ...
        const response = {
          success: false,
          message: error.message || "Unable to mark delivered",
        };
        if (typeof callback === "function") return callback(response);
        socket.emit("conversation_error", response);
      }
    ...
    socket.on("message_seen", async (payload = {}, callback) => {
      try {
        ...
        const response = {
          success: false,
          message: error.message || "Unable to mark seen",
        };
        if (typeof callback === "function") return callback(response);
        socket.emit("conversation_error", response);
      }
